...
import frc.lib.vision.ControlMode.*;
...
public class LimeLight {
    ...
    // run the latency checker periodically every heartBeatPeriod
    Notifier heartBeat = new Notifier(new PeriodicRunnable());
     ...
     * PiPMain - The secondary camera stream is placed in the lower-right corner of
     * the primary camera stream
     * PiPSecondary - The primary camera stream is placed in the lower-right corner
     ...
     * 
     * on - Stop taking snapshots
     * off - Take two snapshots per second
     * 
     * @param snapshot
     */
    ...
    public void LLtoggleLed() {
        ...
        if (mode == LedMode.forceOn || mode == LedMode.pipeline)
            setLEDMode(LedMode.forceOff);
        else if (mode == LedMode.forceOff)
